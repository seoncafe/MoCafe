...
! local_variables
  ...
  real(kind=wp) :: rmax,xmax,ymax,zmax,r_alpha,z_alpha
...
!--- Read in parameters from params.par using namelist command
  namelist /input_parameters/ no_photons,hgg,albedo,luminosity,distance_unit,&
                              out_file,nprint,tau_crit,wgt_min,psf_file,left_right_fold,output_mode,iseed
  namelist /geometry_parameters/ nx,ny,nz,rmax,zmax,r_alpha,z_alpha,dust,source
