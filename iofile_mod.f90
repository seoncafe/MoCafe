...
!=============================================================================
  ...
  subroutine io_append_5D_real64(file, array, status, bitpix)
    type(io_file_type), intent(inout) :: file
    real(real64),       intent(in)    :: array(:,:,:,:,:)
    integer,            intent(inout) :: status
    integer, optional,  intent(in)    :: bitpix
    ...
    case (IO_FMT_HDF5)
       if (present(bitpix)) then; call hdf5_append_5D_real64(file%hdf5, array, status, bitpix)
       else;                       call hdf5_append_5D_real64(file%hdf5, array, status); endif
    case default; status = IO_ERR_UNKNOWN_FMT
    end select
  end subroutine io_append_5D_real64
