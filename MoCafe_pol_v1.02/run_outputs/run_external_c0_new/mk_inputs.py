...
sample_list = np.arange(10)+1
mach_list   = np.array([1.0])
tau_list    = (np.arange(20)+1)*0.01
...
for nsample in sample_list:
  for tau in tau_list:
    file_in      = "M%03d_%03d%s_t%04d" % (mach*10, nsample, band, tau*1000) + ".in"
    if mach == 0:
       density_file = ''
    else:
