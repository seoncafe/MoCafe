...
!    - intrinsic random_number routine is not thread-safe with openmp, so should not be used.
...
!
! History:
!   v2.44 (2020/09/04) resonance_vz_rascas is added.
...
!-----
  ...
  public rand_resonance_vz_seon, rand_resonance_vz_seon2, rand_resonance_vz_seon3
  ...
  interface rand_resonance_vz
     !module procedure rand_resonance_vz_laursen
     !module procedure rand_resonance_vz_seon
     module procedure rand_resonance_vz_seon3
  end interface
  ...
  subroutine init_random_kiss_seed(iseed)
    ...
    if (.not. set_iseed) then
       ...
       if (istat == 0) then
          ...
          pid = pid + getpid() + 9999*omp_get_thread_num()
#endif
#ifdef MPI
          pid = pid + getpid()
#endif
...
!==============================
  ...
  real(kind=wp) :: x0,vz
  real(kind=wp) :: A0,A1,A2,Atot
  real(kind=wp) :: u0,u1,x1
  ...
  real(kind=wp), parameter :: x0_crit = 0.8_wp
  real(kind=wp) :: h0,h1,hmax
  real(kind=wp) :: x,t1,t2,del_t,u ,yy
  real(kind=wp) :: rr, r0, r1
  ...
  if (x0 <= x0_crit) then
     ...
     u0 = exp(-x0**2/2.0_wp)
     u1 = u0 + sqrt(two_over_PI*a*(1.0_wp - u0)*u0*x0)
     !--- 2019-08-19, numerical bug.
     !if (u1 > 0.0_wp) then
     if (u1 > EPS_SP) then
        h0 = 0.5_wp * u0
        !----- first area
        A0 = h0 * u0
        !----- second area
        x1 = sqrt(-2.0_wp*log(u1))
        A1 = (u1 - u0)*h0
        !----- third area
        !h1 = u1/PI * (atan((x1-x0)/a)+atan((x1+x0)/a))
        !h1 = u1/PI * atan(2.0_wp*a*x1/(a**2+(x0**2-x1**2)))
        h1 = a * two_over_PI * u1 *x1/(x0**2 - x1**2)
        if (x0 > xc) then
           hmax = a*0.3861_wp/(x0**2 - 1.373_wp)
           h1   = maxval([h1, hmax])
        endif
        A2   = (1.0_wp - u1)*h1
        Atot = A0 + A1 + A2
        ...
        do while(.true.)
           rr = rand_number()
           if (rr <= r0) then
              !-- using two randome number is slightly faster than using sqrt. No.
              !u  = u0*(1d0-abs(rand_number()-rand_number()))
              u  = u0*sqrt(rand_number())
              yy = u  * rand_number()
           else if (rr <= r1) then
              u  = (u1-u0)*rand_number() + u0
              yy = h0 * rand_number()
           else
              u  = (1.0_wp-u1)*rand_number() + u1
              yy = h1 * rand_number()
           endif
           !if (rr > r1) then
           !   u  = (1.0_wp-u1)*rand_number() + u1
           ...
           !else
           !   !-- using two randome number is slightly faster than using sqrt. No.
           !   !u  = u0*(1.0_wp-abs(rand_number()-rand_number()))
           !   u  = u0*sqrt(rand_number())
           !   yy = u  * rand_number()
           !endif
           x  = sqrt(-2.0_wp*log(u))
           ...
           t1 = atan((-x-x0)/a)
           del_t = t2 - t1
           if (u /PI * del_t >= yy) exit
        enddo
        vz = x0 + a * tan(del_t * rand_number() + t1)
     else
        hmax = a*0.3861_wp/(x0**2 - 1.373_wp)
        do while(.true.)
           u  = rand_number()
  ...
  !=================================================
  function rand_resonance_vz_seon2(x0in,a) result(vz)
  implicit none
  real(kind=wp), intent(in) :: x0in, a
  real(kind=wp) :: vz
  real(kind=wp) :: x0,x0sq,api,beta0,one_b0,beta
  real(kind=wp) :: A1,A2,Atot
  real(kind=wp) :: pb,Cb
  real(kind=wp), parameter :: x0_crit = 1.0_wp
  real(kind=wp), parameter :: xc = 1.0_wp + sqrt(2.0_wp)
  real(kind=wp), parameter :: PI = 3.141592653589793238462643383279502884197_wp
  real(kind=wp) :: h0,h2,hmax,h0_two
  ...
  if (x0 < xc) then
     ...
     Atot = A1 + A2
     do while(.true.)
        if (rand_number() < A1/Atot) then
           !beta = beta0 * (1.0_wp - abs(rand_number() - rand_number()))
           beta = beta0 * sqrt(rand_number())
           Cb   = beta/a
        else
           !beta = beta0 + one_b0 * abs(rand_number() - rand_number())
           beta = 1.0_wp - one_b0 * sqrt(rand_number())
           Cb   = h0 * (1.0_wp - beta)/one_b0
        endif
  ...
  if (x0in < 0.0_wp) vz = -vz
  end function rand_resonance_vz_seon2
  !=================================================
  function rand_resonance_vz_seon3(x0in,a) result(vz)
  implicit none
  real(kind=wp), intent(in) :: x0in, a
  real(kind=wp) :: vz
  ...
  if (x0in < 0.0_wp) vz = -vz
  end function rand_resonance_vz_seon3
  !----------------------------------------------------------------
