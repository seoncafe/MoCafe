...
def radial_profile(data, center=None, normalize=True, whole_area=False):
    ...
    r    = np.floor(r - roff).astype(np.int32)

    #wok  = np.where(~np.isnan(data))
    #tbin = np.bincount(r[wok].ravel(), weights=data[wok].ravel())
    #nr   = np.bincount(r[wok].ravel())
    #rarr = np.unique(r[wok])
    #rmax = np.max(x - center[0])
