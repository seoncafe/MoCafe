...
\file{docs/MoCafe\_amr.pdf}; the dust-emission modes run on the Cartesian grid.
...
$\approx n_\lambda\times n_{\rm cell}\times 8$~bytes per MPI rank; the plain
Cartesian grid only.
...
\bottomrule
\end{longtable}
Read and convert the outputs with \file{python/mocafe\_io.py}
(\code{info}, \code{peek}, \code{convert}).
