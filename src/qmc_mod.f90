...
! photon launching.
!
! Self-contained; no external libraries.  The launch of a stellar packet is
! a low-dimensional integral (origin, source component, wavelength bin,
! direction, and, for external illumination, entry-surface variables); a
! scrambled Sobol net fills that cube more uniformly than independent draws,
! reducing launch noise while keeping every post-launch transport draw on the
! existing Mersenne Twister.  See docs/QUASI_RANDOM_LAUNCH_MOCAFE.md.
!
...
! par%qmc_seed, so a replicate is reproducible from the input seed and fixed
! across the nonlinear (Lucy dust-emission) iterations (common random numbers).
!
