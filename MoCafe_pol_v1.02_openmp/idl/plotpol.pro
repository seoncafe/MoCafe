...
;
; CALLING SEQUENCE:
;  plotpol, x,y , xvec, yvec, [linecolor = linecolor, _extra = extra,/over]
;
; INPUTS:
...
;-
pro plotpol, x, y, xvec, yvec, linecolor = linecolor, $
             over = over, _extra = extra, veclinestyle = veclinestyle, linethick = linethick
compile_opt idl2
on_error, 2

if n_params() ne 4 then begin
   print, 'plotpol calling sequence:'
   print, ' plotpol, x, y, xvec, yvec, [linecolor = linecolor, _extra = extra]'
   return
endif
...
for i = 0, nx-1, 1 do begin
   oplot, x[i] + xvec[i]*[-0.5d0, 0.5d0], y[i] + yvec[i]*[-0.5d0, 0.5d0], color = linecolor, $
